프로젝트 레퍼런스는 TypeScript 프로그램을 더 작은 조각으로 구조화하게 해주는 TypeScript 3.0의 새로운 기능입니다.

이 기능을 사용함으로써 빌드 시간을 크게 향상시킬 수 있고, 컴포넌트 사이의 논리적인 분리를 시행하며, 코드를 새롭고 더 나은 방법으로 구성할 수 있습니다.

TypeScript 빌드를 빠르게 하기 위해 프로젝트 레퍼런스와 함께 동작하는 `tsc`를 위한 새로운 모드인 `--build` 플래그를 소개합니다.

# 예제 프로젝트 (An Example Project)

아주 일반적인 프로그램을 보고 프로젝트 레퍼런스가 어떻게 잘 구성할 수 있게 도와주는지 살펴보겠습니다.
...
명령줄에 전달한 파일의 순서에 대해서는 걱정하지 마세요 - `tsc`가 필요하면 재배열하기 때문에 의존성이 언제나 먼저 빌드 됩니다.
