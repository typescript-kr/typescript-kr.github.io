...
TypeScript는 인터페이스, 상속, 정적 메서드 구현과 같은 많은 일반적인 양식을 지원합니다.
...
C#과 Java에서 주어진 값과 객체는 원시 타입인 ‘null’ 또는 정의된 클래스 타입 중 정확하게 하나의 타입을 가집니다.
...
예를 들어, C#에서는 ‘string’과 ‘int’가 *둘 다 가능한* 타입이 전혀 없기 때문에 이 값을 인자로 전달하는 것은 이상합니다.

TypeScript에서 모든 타입이 단순히 집합이라는 것을 깨닫는 순간 이는 매우 자연스러워집니다.
‘string’ 집합 또는 ‘number’ 집합에 속할 수 있는 값을 어떻게 설명하시겠습니까?
이것은 단순히 그 집합들의 *유니언*: ‘string | number’에 속합니다.
