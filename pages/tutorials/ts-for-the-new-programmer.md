...
당신의 첫 번째 언어로 TypeSript를 고른것을 축하드립니다 - 당신은 이미 좋은 결정을 했습니다!

당신은 이미 Typescript가 Javascript의 "기호(flavor)"나 "변형(variant)"이라는 것을 이미 들어봤을 겁니다.
TypeScript(TS)와 JavaScript(JS)의 관계는 현대 프로그래밍 언어에서 꽤 독특한 관계이니, 이 관계를 더 배우는 것은 당신이 어떻게 TypeScript를 JavaScript에 추가할지에 도움이 될 것 입니다.
...
때문에, 초기 웹 브라우저들은 수십 줄 이상의 코드들을 실행하는데 오래 걸렸습니다.
그러나 시간이 흘러, JS가 점점 더 유명해지며, 웹 개발자들은 JS를 이용해 상호작용을하는 경험들을 만들어내기 시작했습니다.
...
현대 브라우저 웹사이트에서, 당신의 브라우저는 수십만 줄의 코드로 구성된 어플리케이션을 자주 실행합니다.
이는 정적 페이지의 간단한 네트워크로 시작해서, 모든 종류의 만족스러울만한 _어플리케이션_을 위한 플랫폼으로 성장한 "웹"의 길고 점진적인 성장입니다.

이외에도, JS는 node.js를 사용하여 JS 서버들을 구현하기와 같은, 브라우저 맥락에서 벗어나는 일에 사용하기 충분할 정도로 유명해졌습니다.
...
* JavaScript의 동일 연산자는 (`==`) 그것의 인수를 _강제로 변환하여(coerces)_, 예기치 않은 동작을 유발합니다:
...
* JavaScript는 또한 없는 프로퍼티의 접근을 허용합니다:
...
대부분의 프로그래밍 언어들이 이런 종류의 오류들이 발생하면 오류를 표출해주고, 일부는 코드가 실행되기 전인 컴파일 중에 오류를 표출해줍니다.
