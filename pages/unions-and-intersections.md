...
하지만, 더 많은 타입들을 모델링할수록 처음부터 타입을 만들어내기보다는 이미 존재하는 타입들을 구성하거나 결합하는 방법들을 찾게 될 것입니다.

교차 타입과 유니언 타입은 타입들을 구성할 수 있는 방법들 중 하나입니다.
...
기존 방법의 `padLeft`에서 좋은 점 중 하나는 원시값을 단지 전달할 수 있다는 것입니다.
즉 사용법이 간단하고 간결합니다.
...
유니언 타입의 값이 있으면, 유니언에 있는 모든 타입에 공통인 멤버들에만 접근할 수 있습니다.
...
만약 값이 `A | B` 타입을 갖고 있으면, _확신할_ 수 있는 것은 그 값이 `A`_와_`B` 둘 다 가지고 있는 멤버들을 갖고 있다는 것 뿐입니다.
...
유니언을 사용하는 데 있어서 일반적인 기술은 TypeScript가 현재 가능한 타입 추론의 범위를 좁혀나가게 해줄 수 있는 리터럴 타입을 갖는 단일 필드를 사용하는 것입니다. 예를 들어, 하나의 공통 필드가 있는 세 타입을 가지는 유니언을 만들 수 있습니다.
...
function networkStatus(state: NetworkState): string {
  ...
  // state에 swtich문을 사용하여, TypeScript는 코드 흐름을 분석히면서
  // 유니언 타입을 좁혀나갈 수 있습니다.
  switch (state.state) {
    case "loading":
      return "Downloading...";
